...
public class SameQuantSameInstructionRule implements IRuleStrategy {
	...
	/**
	 * group by same quantity and then same transaction details, if MORE THAN
	 * one value found in the grouping list, then rule violation success.
	 * 
	 * @throws SAPException
	 *             .
	 */
		...
		try {
			...
			for (Iterator it = map.values().iterator(); it.hasNext();) {
				...
				for (Iterator it2 = mp.values().iterator(); it2.hasNext();) {
					...
					String grpOneGrpTwoViolated = "Same Quantity Same Instruction Rule Transaction Code Rule violated for"
							+ groupIds.toString();

					if (groupIds.size() > 1) { // if MORE THAN one value found
												// in the grouping list
						GroupOrderWarningResponse groupOrderWarningResponse = new GroupOrderWarningResponse(
								grpOneGrpTwoViolated, groupIds);
